...
def preprocess_image(img):
    ...
    denoised = cv2.fastNlMeansDenoisingColored(enhanced, None,
                                               ...
                                               ENHANCE_PARAMS['denoise_strength'], 7, 21)
    gray = cv2.cvtColor(denoised, cv2.COLOR_BGR2GRAY)
    return cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                 cv2.THRESH_BINARY, ENHANCE_PARAMS['adaptive_block'], 11)
...
def detect_text(cv_image):
    ...
    # 预处理（EAST模型输入）
    blob = cv2.dnn.blobFromImage(frame, 1.0, (TEXT_MODEL_PARAMS['width'], TEXT_MODEL_PARAMS['height']), 
    ...
    # 解码检测结果
    [boxes, confidences] = decodeBoundingBoxes(scores, geometry, TEXT_MODEL_PARAMS['thr'])
    
    # 应用NMS
    indices = cv2.dnn.NMSBoxesRotated(boxes, confidences, TEXT_MODEL_PARAMS['thr'], TEXT_MODEL_PARAMS['nms'])
    ...
    for i in indices if indices is not None else []:
        ...
        # CRNN输入预处理
        blob_ocr = cv2.dnn.blobFromImage(cropped, scalefactor=1/127.5, size=(100, 32),
...
def main_detection():
    ...
    print("先尝试二维码识别...")
    qr_result = decode_qrcode(img)
    if qr_result:
        print(f"检测到二维码: {qr_result}")
        return
    
    print("再尝试文本识别...")
    text_results = detect_text(img)
